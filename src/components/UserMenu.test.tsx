...
describe('UserMenu', () => {
  it('renders an avatar button with the first initial of the email', () => {
    render(<UserMenu user={mockUser} onSignOut={vi.fn()} onOpenApiKeys={vi.fn()} />);
  ...
  it('calls onSignOut when Sign out is clicked', () => {
    ...
    render(<UserMenu user={mockUser} onSignOut={onSignOut} onOpenApiKeys={vi.fn()} />);
    fireEvent.click(screen.getByRole('button', { name: /account menu/i }));
  ...
  it('uses uppercase first letter of email as initial', () => {
    ...
    render(<UserMenu user={user} onSignOut={vi.fn()} onOpenApiKeys={vi.fn()} />);
